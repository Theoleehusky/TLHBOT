        ...
        for event in self.stream:
            if event['type'] == 'gameState':
                if event['status'] == 'mate' or event['status'] == 'resigned':
                    print('game ended')
                    break
                elif event['status'] == 'started' and event['moves'].count(' ') % 2 == spaces:
                    self.move(event['moves'])
            elif event['type'] == 'chatLine':
                print(event)

    def move(self, game_state):
        self.board.push_uci(game_state.split(' ')[-1])
        if self.opening:
            ...
            print("Endgame")
            move = requests.get('http://tablebase.lichess.ovh/standard?fen=' + self.board.fen()).json()['moves'][0]['uci']
        else:
            move = self.minimax(self.deep, True, -1000, 1000, True)
    ...
    def minimax(self, n, maxmin, alpha, beta, move):
        ...
        if move:
            ...
            for move in (str(x) for x in self.board.legal_moves):
                ...
                #     self.board.push_uci(move)
                #     value = self.minimax(1, False, alpha, beta, False)
                # else:
                self.board.push_uci(move)
        ...
        if maxmin:
            ...
            for move in (str(x) for x in self.board.legal_moves):
                ...
                # if self.board.is_capture(x) or self.board.gives_check(x):
                #     self.board.push_uci(move)
                #     value = self.minimax(1, True, alpha, beta, False)
                # else:
                self.board.push_uci(move)
